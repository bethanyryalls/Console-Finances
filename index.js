...
// calculating the greatest decrease in losess

// setting values for biggest decrease and biggest decrease index and date
var biggestDecrease = 0;
var biggestDecreaseIndex = 0;
var biggestDecreaseDate = 0;

for (var i = 0; i < finances.length; i++) {
    var prevDecrease;
    if (i > 0) {
        prevDecrease = finances[i - 1][1];
    } else {
        prevDecrease = 0;
    }

    // calculating difference between current and previous months, if current is higher decrease then replace biggestDecrease
    var currentDecrease = finances[i][1] - prevDecrease;
    if (currentDecrease < biggestDecrease) {
        biggestDecrease = currentDecrease;
        biggestDecreaseIndex = i;
        biggestDecreaseDate = finances[i][0];
    }
}

console.log("Greatest decrease in losses: " + biggestDecreaseDate + " ($" + biggestDecrease + ")");
